		...
		<div slot="sys-form" style="height: 100%; position:fixed;">
			...
			<v-form>
				<v-form-group :label="labelTranslation('Terminal nr') | capitalize">
					<v-input :containerStyle="'width: 30%'" v-model="model.Id" :isEditMode="isChanged" :disabled="!canEdit" name="TerminalNr" :validate="getSysTerminalValidationRulesFor('Id')"></v-input>
				</v-form-group>
				<v-form-group :label="labelTranslation('Terminal name') | capitalize">
					<v-input v-model="model.Description" name="TerminalName" :isEditMode="isChanged" :disabled="!canEdit" :validate="getSysTerminalValidationRulesFor('Description')"></v-input>
				</v-form-group>
				<v-form-group :label="labelTranslation('Driver type') | capitalize">
					<v-select v-model="model.DriverId" :isEditMode="isChanged" :options="terminalDriverKeyValues" :disabled="!canEdit" name="DriverType" :validate="getSysTerminalValidationRulesFor('DriverId')"></v-select>
				</v-form-group>
				<v-form-group :label="labelTranslation('Terminal type') | capitalize">
					<v-select v-model="model.TypeId" :isEditMode="isChanged" :options="terminalTypeKeyValues" :disabled="!canEdit" name="TerminalType" :validate="getSysTerminalValidationRulesFor('TypeId')"></v-select>
				</v-form-group>
				<tr>
			...
			<div style="height: calc(77% - 230px); overflow-x: hidden; overflow-y: auto; position:relative;">
			<v-form>
				...
				<v-form-group :label="labelTranslation('CVE number') | capitalize">
					<v-input :containerStyle="'width: 40%'" :isEditMode="isChanged" :disabled="!canEdit" v-model="model.CVENumber" name="CVENumber" :validate="getSysTerminalValidationRulesFor('CVENumber')"></v-input>
				</v-form-group>
				...
				<v-form-group :label="labelTranslation('Address') | capitalize" v-if="!isVirtual">
					<v-input v-model="model.SysTerminalTMCSetting.Address" :isEditMode="isChanged" :disabled="!canEdit" name="Address" :validate="getSysTerminalTMCSettingValidationRulesFor('Address')"></v-input>
				</v-form-group>
				<v-form-group :label="labelTranslation('Parameters') | capitalize">
					<v-input v-model="model.Parameter" name="Parameter" :isEditMode="isChanged" :disabled="!canEdit" :validate="getSysTerminalValidationRulesFor('Parameter')"></v-input>
				</v-form-group>
				<v-form-group :label="labelTranslation('Entry direction') | capitalize">
					<v-select v-model="model.AccessBookingId" :isEditMode="isChanged" :disabled="!canEdit" :options="terminalAccessBookingKeyValues" :optionsCapitalize="false" name="AccessBooking" :validate="getSysTerminalValidationRulesFor('AccessBookingId')"></v-select>
				</v-form-group>
				<v-form-group :label="labelTranslation('Time direction') | capitalize">
					<v-select v-model="model.TimeBookingId" :isEditMode="isChanged" :disabled="!canEdit" :options="terminalTimeBookingKeyValues" :optionsCapitalize="false" name="TimeBooking" :validate="getSysTerminalValidationRulesFor('TimeBookingId')"></v-select>
				</v-form-group>
				...
				<v-form-group :label="labelTranslation('Ring ID') | capitalize">
					<v-input :containerStyle="'width: 30%'" :isEditMode="isChanged" :disabled="!canEdit" v-model="model.RingId" name="RingId" :validate="getSysTerminalValidationRulesFor('RingId')"></v-input>
				</v-form-group>
				...
				<v-form-group :label="labelTranslation('Entry release') | capitalize" v-if="isTMC">
					<v-select v-model="model.SysTerminalTMCSetting.AccessSettingId" :disabled="!canEdit" :isEditMode="isChanged" :options="terminalAccessSettingKeyValues" name="AccessSetting" :validate="getSysTerminalTMCSettingValidationRulesFor('AccessSettingId')"></v-select>
				</v-form-group>
				<v-form-group :label="labelTranslation('IP address') | capitalize" v-if="isTMC">
					<v-input v-model="model.SysTerminalTMCSetting.IPAddress" :disabled="!canEdit" :isEditMode="isChanged" name="IPAddress" :validate="getSysTerminalTMCSettingValidationRulesFor('IPAddress')"></v-input>
				</v-form-group>
				<v-form-group :label="labelTranslation('Whitelist') | capitalize" v-if="isTMC">
					<v-select v-model="model.SysTerminalTMCSetting.WhitelistSettingId" :disabled="!canEdit" :isEditMode="isChanged" :options="terminalWhitelistSettingKeyValues" name="Whitelist" :validate="getSysTerminalTMCSettingValidationRulesFor('WhitelistSettingId')"></v-select>
				</v-form-group>
				<v-form-group :label="labelTranslation('Repeat window') | capitalize" v-if="isTMC">
					<v-input :styleClass="'width: 30%'" :isEditMode="isChanged" :disabled="!canEdit" v-model="model.SysTerminalTMCSetting.PassBackInterval" name="PassBackInterval" :validate="getSysTerminalTMCSettingValidationRulesFor('PassBackInterval')"></v-input>
				</v-form-group>
				<v-form-group :label="labelTranslation('Callback contacts') | capitalize" v-if="isTMC">
					<v-select v-model="model.SysTerminalTMCSetting.DoorMonitoringId" :disabled="!canEdit" :isEditMode="isChanged" :options="terminalDoorMonitoringKeyValues" name="DoorMonitoring" :validate="getSysTerminalTMCSettingValidationRulesFor('DoorMonitoringId')"></v-select>
				</v-form-group>
				<v-form-group :label="labelTranslation('Timeout if late') | capitalize" v-if="isTMC">
					<v-input :styleClass="'width: 30%'" :isEditMode="isChanged" :disabled="!canEdit" v-model="model.SysTerminalTMCSetting.DoorMonitoringTimeout" name="DoorMonitoringTimeout" :validate="getSysTerminalTMCSettingValidationRulesFor('DoorMonitoringTimeout')"></v-input>
				</v-form-group>
				<v-form-group :label="labelTranslation('Relais time') | capitalize" v-if="isTMC">
					<v-input :styleClass="'width: 30%'" :isEditMode="isChanged" :disabled="!canEdit" v-model="model.SysTerminalTMCSetting.DoorOpenTimeForAll" name="DoorOpenTimeForAll" :validate="getSysTerminalTMCSettingValidationRulesFor('DoorOpenTimeForAll')"></v-input>
				</v-form-group>
				<v-form-group :label="labelTranslation('Connection') | capitalize" v-if="isTMC">
					<v-checkbox id="Connection" :isEditMode="isChanged" :disabled="!canEdit" v-model="model.SysTerminalTMCSetting.OnLineSetting" name="Connection" :validate="getSysTerminalTMCSettingValidationRulesFor('OnLineSetting')"></v-checkbox>
				</v-form-group>
				<v-form-group :label="labelTranslation('Costcenter in') | capitalize" v-if="isTMC">
					<v-select v-model="model.SysTerminalTMCSetting.CostCenterIn" :disabled="!canEdit" :isEditMode="isChanged" :options="sysCostCenterKeyValues" name="CostCenterIn" :validate="getSysTerminalTMCSettingValidationRulesFor('CostCenterIn')"></v-select>
				</v-form-group>
				<v-form-group :label="labelTranslation('Costcenter out') | capitalize" v-if="isTMC">
					<v-select v-model="model.SysTerminalTMCSetting.CostCenterOut" :disabled="!canEdit" :isEditMode="isChanged" :options="sysCostCenterKeyValues" name="CallbackContact" :validate="getSysTerminalTMCSettingValidationRulesFor('CostCenterOut')"></v-select>
				</v-form-group>
			</v-form>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		checkDriverType () {
			var id1 = this.tmcIds.find(id => id === this.model.DriverId)
			var id2 = this.virtualIds.find(id => id === this.model.DriverId)
			var id3 = this.crosspointIds.find(id => id === this.model.DriverId)
			if (id1 !== undefined) {
				this.isCrosspoint = false
				this.isTMC = true
				this.isVirtual = false
